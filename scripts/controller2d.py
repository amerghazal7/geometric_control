...
class Controller2D(object):
    def __init__(self, waypoints):
        ...
        self._conv_rad_to_steer  = 180.0 / 40.0 / np.pi
        self._pi                 = np.pi
        self._2pi                = 2.0 * np.pi
    ...
    def update_controls(self, dt):
        ...
        if self._start_control_loop:
            ...
            Ithrottle = v_total_error*Ki_throttle
            Dthrottle = v_error_rate*Kd_throttle
            throttle_output = Pthrottle + Ithrottle + Dthrottle
            brake_output    = 0
            # print(f"v_current_error: {v_current_error}")
            # print(f"v_total_error: {v_total_error}")
            # print(f"v_error_rate: {v_error_rate}")
            # print(f"self.vars.v_total_error: {self.vars.v_total_error}")
            # print(f"self.vars.v_previous_error: {self.vars.v_previous_error}")
            # print(f"Pthrottle: {Pthrottle}")
            # print(f"Ithrottle: {Ithrottle}")
            # print(f"Dthrottle: {Dthrottle}")
            print(f"throttle_output: {throttle_output}")
            print(f"v: {v}")
            print("==========================================")
        ...
        idx = 0
        for wp in waypoints:
            dist = math.sqrt((wp[0] - x_rear)**2 + (wp[1] - y_rear)**2)
        ...
        if idx == len(waypoints):
            carrot = waypoints[0]
        alpha = math.atan2(carrot[1] - y_rear, carrot[0] - x_rear) - yaw

        steer_output    = math.atan2(2 * L * np.sin(alpha), lookahead_distance)
...
class Controller2D(object):
    def __init__(self, waypoints):
        ...
        self._set_steer          = 0
        self._waypoints          = waypoints
        self._conv_rad_to_steer  = 180.0 / 20.0 / np.pi
        self._pi                 = np.pi
        self._2pi                = 2.0 * np.pi
    ...
    def update_controls(self, dT = 0.0001):
        ######################################################
        # RETRIEVE SIMULATOR FEEDBACK
        ######################################################
        x               = self._current_x
